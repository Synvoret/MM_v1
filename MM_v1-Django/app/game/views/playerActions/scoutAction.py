...
from dataset.utils.dataset.decorators.choices import HITS, HIT_LOCATIONS
...
def scoutAction(request):
    ...
    player_colour = request.session['playerColourActive'] # as string for example 'yellow'
    ...
    if request.method == 'GET':
        ...
        # scouting merchant
        if request.GET.get('type_request') == 'scouting merchant':
            data['skillName'] = 'Scouting'
            data['skillValue'] = game_captains.skill_value(player_colour, 'scouting')

        if request.GET.get('type_request') == 'merchant':
            merchantToken = MerchantTokens.objects.get(nationality=ships_in_zone.merchants_ship[player_localisation])
            data['merchantLocalisation'] = str(player_localisation).lower().replace(' ', '-')
            data['merchantTokenAwersImageUrl'] = merchantToken.awers.url
    ...
    if request.method == 'POST':
        ...
        if request.POST.get('type_request') == 'merchant raid':
            ...
            for field in merchants_track._meta.fields:
                ...
                if field_name == 'id' or field_name == 'game_number':
                    ...
                    player_bounties_list.append(merchantToken.nationality)
                    ...
                    data['merchantToken'] = merchantToken.awers.url # put metchant token on merchant track
            ...
            data['playerCaptainSeamanshipValue'] = game_captains.skill_value(player_colour, 'seamanship')  # .seamanship: get SEAMANSHIP Captain parameter as int
        ...
        if request.POST.get('type_request') == 'merchant trade':
            ...
            # captain influence
            data['playerCaptainInfluenceValue'] = game_captains.skill_value(player_colour, 'influence')
